...
<aside class="fixed md:static inset-y-0 left-0 z-40 
           ...
           md:translate-x-0  font-serif" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"> <!-- LOGO -->
    ...
    <nav class="flex-1 py-6 px-4 space-y-2 overflow-y-auto">
        ...
        <div x-data="{ open: {{ request()->routeIs('homeModal.*') || request()->routeIs('memories.*') || request()->routeIs('topAchievers.*') || request()->routeIs('news.*') || request()->routeIs('videos.*') || request()->routeIs('socials.*') ? 'true' : 'false' }} }"
            ...
            <button @click="open = !open"
                ...
                <svg :class="{ 'rotate-180': open }" class="w-5 h-5 transition-transform duration-200" fill="none"
        ...
        <!-- Donation Dropdown -->
        <div x-data="{ open: {{ request()->routeIs('achievements.*') || request()->routeIs('fundRaise.*') || request()->routeIs('donationBooking.*') || request()->routeIs('donationImage.*') ? 'true' : 'false' }} }"
            class="space-y-1">
            <button @click="open = !open"
                class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl font-medium text-gray-700 transition-all duration-200
                           hover:bg-blue-100 hover:text-[#00285E]
        ...
        <div class="space-y-2">
            ...
            open: {{ request()->routeIs('jobPost.*') || request()->routeIs('jobApp.*') ? 'true' : 'false' }}
                ...
                <!-- Parent Button -->
                <button @click="open = !open" class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl font-medium
