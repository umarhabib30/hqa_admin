...
@section('content')
    <div class="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow">
        ...
        <h2 class="text-2xl font-semibold mb-6">Create Alumni Event</h2>

        <form method="POST" action="{{ route('alumniEvent.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf

            <input name="title" placeholder="Event Title" class="w-full border p-3 rounded">

            <textarea name="description" rows="3" placeholder="Event Description" class="w-full border p-3 rounded"></textarea>
