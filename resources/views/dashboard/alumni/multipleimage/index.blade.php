...
<div>
    <div class="flex justify-between mb-6 md:flex-row flex-col">
        <h1 class="text-2xl font-semibold">Alumni Gallery</h1>

        <a href="{{ route('alumniImages.create') }}"
    ...
    <div class="bg-white rounded-xl shadow p-6 mb-6">
        ...
        <div class="flex gap-3">
            <a href="{{ route('alumniImages.edit',$gallery->id) }}"
                class="px-5 py-2 border border-[#00285E] text-[#00285E]
                      rounded-lg hover:bg-[#00285E] hover:text-white transition">
                Edit
            </a>

            <form method="POST" action="{{ route('alumniImages.destroy',$gallery->id) }}">
                @csrf
                @method('DELETE')
                <button
