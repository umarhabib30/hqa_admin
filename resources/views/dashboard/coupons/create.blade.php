    ...
    <form method="POST" action="{{ route('coupons.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <select id="coupon_type" name="coupon_type" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#00285E] focus:border-transparent @error('coupon_type') border-red-500 @enderror">
                <option value="percentage" {{ old('coupon_type', 'percentage') === 'percentage' ? 'selected' : '' }}>Percentage (Default)</option>
                <option value="amount" {{ old('coupon_type') === 'amount' ? 'selected' : '' }}>Amount (Fixed Value)</option>
            </select>
            @error('coupon_type')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
